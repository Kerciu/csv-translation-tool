...
impl BatchTranslator {
    pub fn new(
        ...
        let model =
            MTModel::new(&config.model_config, vb).context("Failed to initialize Marian model")?;
    ...
    pub fn translate_batch(
        ...
        let mut decoder_input_ids =
            Tensor::new(&[[start_token]], &self.device)?.repeat((input_tensor.dim(0)?, 1))?;
        ...
        for _ in 0..max_length {
            ...
            decoder_input_ids = Tensor::new(next_tokens.clone(), &self.device)?.unsqueeze(1)?;
            ...
            if outputs
                .iter()
                .all(|seq| seq.contains(&self.model_config.config().eos_token_id))
            {
                break;
            }
        }
