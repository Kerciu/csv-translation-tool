...
def check_file_size(sf_filename, pt_filename):
    ...
    if (sf_size - pt_size) / pt_size > 0.01:
        raise RuntimeError(
            f"File size difference exceeds 1% between {sf_filename} and {pt_filename}"
        )
...
def copy_additional_files(source_folder, dest_folder):
    ...
    for file in os.listdir(source_folder):
        ...
        if os.path.isfile(file_path) and not (
            file.endswith(".bin") or file.endswith(".py")
        ):
...
def convert_files(source_folder, dest_folder, delete_old):
    ...
    if not index_file:
        raise RuntimeError(
            "Index file not found. Please ensure the correct folder is specified."
        )
...
def convert_pt_to_safetensors(
    src_lang, tgt_lang, source_folder, dest_folder=None, delete_old=False
):
    if not dest_folder:
