...
const CSVTable = ({
  ...
  const handleCellEditClick = (rowIdx: number, colIdx: number, value: string) => {
    /* edit */
    if (isEditable && selectedColumns.includes(headers[colIdx]) && selectedRows.includes(rowIdx)) {
      ...
      <Table>
        <TableHeader className='sticky top-0 z-10 bg-background'>
          <TableRow>
            ...
            {headers.map((header, index) => (
              <TableHead
                key={index}
                className={cn(selectedColumns.includes(header) && 'bg-primary/10 font-bold')}
              >
                {header}
              </TableHead>
            ))}
        ...
        <TableBody>
          {data.map((row, rowIndex) => (
            <TableRow
              key={rowIndex}
              className={cn(selectedRows.includes(rowIndex) && 'bg-secondary/20')}
            >
              <TableCell
                className={cn(
